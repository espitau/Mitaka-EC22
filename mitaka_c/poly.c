...
void FFT(poly* p){
  Zf(FFT)(p->coeffs,MITAKA_LOG_D);

}

void invFFT(poly* p){
  Zf(iFFT)(p->coeffs,MITAKA_LOG_D);
}


void pointwise_mul(poly* p1, const poly* p2){
  /* Multiplication in FFT form, result in p1*/
  Zf(poly_mul_fft)(p1->coeffs, p2->coeffs, MITAKA_LOG_D);

}

void poly_add(poly* p1, const poly* p2){
  /* Addition result in p1*/
  Zf(poly_add)(p1->coeffs, p2->coeffs, MITAKA_LOG_D);

}

void poly_sub(poly* p1, const poly* p2){
  /* Subtraction p1-p2 result in p1*/
  Zf(poly_sub)(p1->coeffs, p2->coeffs, MITAKA_LOG_D);
}

void poly_div_FFT(poly* p1, const poly* p2){
  Zf(poly_div_fft)(p1->coeffs, p2->coeffs, MITAKA_LOG_D);
}

void FFT_adj(poly* p){
  Zf(poly_adj_fft)(p->coeffs, MITAKA_LOG_D);
}

void FFT_mul_adj(poly* p1, const poly* p2){
  Zf(poly_muladj_fft)(p1->coeffs, p2->coeffs, MITAKA_LOG_D);
}

void FFT_mul_selfadj(poly *p1){
  Zf(poly_mulselfadj_fft)(p1->coeffs, MITAKA_LOG_D);
}
...
void poly_recenter(poly* p) {
  for(int i=0; i < MITAKA_D; ++i) {
    p->coeffs[i].v = fmod(p->coeffs[i].v, (double) MITAKA_Q);
    if(p->coeffs[i].v > MITAKA_Q/2)
	p->coeffs[i].v -= (double) MITAKA_Q;
    else if(p->coeffs[i].v < -MITAKA_Q/2)
	p->coeffs[i].v += (double) MITAKA_Q;
  }
}
