...
void test_normal(){
  ...
  mean /= (MITAKA_D * ITER);
  stddev = sqrt(stddev / (MITAKA_D * ITER));
  printf("mean   = %f (expecting 0)\nstddev = %f (expecting %f)\n",
		mean, stddev, sqrt((double)MITAKA_D/2));
}
...
{
    poly c1, c2, v1, v2;
    uint16_t buf[MITAKA_D];
    randombytes((uint8_t*)buf, sizeof buf);
    for(int i=0; i<MITAKA_D; i++) {
      c1.coeffs[i].v = buf[i] % MITAKA_Q;
      c2.coeffs[i].v = 0.;
    }
    sampler(sk, &c1, &c2, &v1, &v2);
    poly_sub(&v1,&c1);
    poly_recenter(&v1);
    printf("v1:");print_poly(&v1); printf("v2:");print_poly(&v2);   
    ...
    mean /= MITAKA_D;
    stddev = sqrt(((stddev) / (MITAKA_D))-(mean*mean));
    #if MITAKA_D == 512
      printf("mean   = %f (expecting %f)\nstddev = %f (expecting %f)\n",
      mean, 0., stddev, 299.975692355231);
    #elif MITAKA_D == 1024
    printf("mean   = %f (expecting %f)\nstddev = %f (expecting %f)\n",
      mean, 0., stddev, 340.9479820427744);
    #endif
}
