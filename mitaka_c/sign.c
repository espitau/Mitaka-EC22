...
void H(const uint8_t* m_r, poly* c1){
  ...
  ctr = 0;
  shake128_absorb(state, m_r, MSG_BYTES+MITAKA_K/8);
  while(ctr < MITAKA_D) 
  {
    ...
    for(i=0;i<SHAKE128_RATE && ctr < MITAKA_D;i+=2)
    {
      ...
      if(val < 5*MITAKA_Q)
      {
        c1->coeffs[ctr].v = (double)(val%MITAKA_Q);
        ctr++;
      }
    }
  }
...
void sampler(const secret_key* sk, const poly* c1, const poly* c2, poly* v0, poly* v1){
  ...
  // online
  // first nearest plane
  set_poly(&temp_c1, c1); set_poly(&temp_c2, c2);
  ...
  FFT(x);
  set_poly(v0, x); set_poly(v1, x);
  pointwise_mul(v0, &(sk->b20)); pointwise_mul(v1, &(sk->b21));
  poly_sub(&temp_c1, v0); poly_sub(&temp_c2, v1); // Alg 13 line 14 c1 in the paper is [temp_c1,temp_c2] in the code
  ...
  poly_add(v0, &temp); poly_add(v1, x); // v0 = v1 + x1*b1
...
void sign(const uint8_t* m, const secret_key* sk, signature* s){
  int trials = 0;
  poly c1, c2, v0, v1;
  uint8_t m_r[MSG_BYTES+MITAKA_K/8];
  for(int i=0; i < MSG_BYTES; ++i) m_r[i] = m[i];

  do{
    trials++;
    randombytes(m_r+MSG_BYTES, MITAKA_K/8);
    H(m_r, &c2);
    for(int i=0; i < MITAKA_D; ++i) c1.coeffs[i].v = 0;
    //printf("c1:");print_poly(&c1); printf("c2:");print_poly(&c2);    
    sampler(sk, &c1, &c2, &v0, &v1);
    //printf("v1:");print_poly(&v0); printf("v2:");print_poly(&v1);   

    poly_sub(&c1, &v0); poly_sub(&c2, &v1);
    poly_recenter(&c1);
    poly_recenter(&c2);
  ...
  set_poly(&(s->s1), &c1);
  set_poly(&(s->s2), &c2);
  for(int i=0; i < MITAKA_K/8; ++i) (s->r)[i] = m_r[i+MSG_BYTES];
...
int verify(uint8_t* m, public_key* pk, signature* s){
  ...
  uint8_t m_r[MSG_BYTES+MITAKA_K/8];
  for(int i=0; i < MSG_BYTES; ++i) m_r[i] = m[i];
  for(int i=0; i < MITAKA_K/8; ++i) m_r[i+MSG_BYTES] = (s->r)[i];
  H(m_r, &c1);
  ...
  poly_add(&t, &c1);
  poly_recenter(&t);
  printf("s1*h+c:"); print_poly(&t);
  printf("s2:"); print_poly(&(s->s2));
